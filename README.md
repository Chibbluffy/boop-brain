...
- `POST /lore/summarize_channel` — manually summarizes a channel's current rolling history into guild lore right now, backing `!lore summarize`. See [Conversation summaries](#conversation-summaries) below for how this also happens automatically.
...
Per-exchange auto-extraction (`_extract_and_store` in `main.py`) only ever sees one message + one reply at a time, so it can only ever produce small atomic facts — it can't capture "what a whole conversation was actually about." `app/summarize.py` + `app/idle_sweep.py` add a second, separate kind of memory for that:

- A background loop (`idle_sweep.run_idle_sweep_loop`, started on FastAPI startup) checks every `IDLE_SWEEP_INTERVAL_SECONDS` whether any channel's rolling history is close to expiring (`IDLE_SUMMARY_THRESHOLD_SECONDS` remaining) — i.e. the channel's gone quiet and its history is about to disappear. If so, it summarizes the *entire* rolling history into a few sentences and stores it as guild lore **before** the history is gone, then marks that channel as "already summarized" (via a Redis key with its own TTL) so the same idle period doesn't get summarized repeatedly on every sweep.
- Summaries are stored with `mem0_client.add(..., infer=False, metadata={"type": "summary"})` — `infer=False` skips mem0's own extraction/dedup LLM reasoning entirely, since we've already decided what the summary says ourselves; this also means summaries aren't affected by whatever issue is (still, as of this writing) causing atomic per-exchange extraction to under-report ("nothing extracted from this exchange").
...
- `!lore summarize` (→ `POST /lore/summarize_channel`) triggers this manually and immediately, ignoring the idle-sweep's "already summarized" marker — useful for capturing something notable without waiting for the channel to go quiet.
- Requires `history.set_channel_guild` to have run at least once for a channel (it's called on every `/generate`) — the idle-sweep needs to know which guild a channel belongs to, since Redis history keys are only keyed by `channel_id`.
